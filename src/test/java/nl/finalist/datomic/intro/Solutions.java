...
public class Solutions
{
    ...
    public static final String query2 = "[:find ?p :in $ :where " + 
                                        "[?p :person/height]]";
    public static final String query3 = "[:find ?t ?s :in $ ?n :where " +
                                        "[?p :name ?n]" +
                                        "[?p :player/salary ?s]" +
                                        "[?p :player/team ?team]" + 
                                        "[?team :name ?t]]";
    public static final String query4 = "[:find ?instant :in $ ?n :where " + 
                                        "[?p :player/salary _ ?tx]" +
                                        "[?tx :db/txInstant ?instant]]";    
    //  database argument should be: conn.db().asOf( year2011 )
    public static final String query5 = "[:find ?s ?c :in $ :where " +
                                        "[?t :twitter/screenName ?s]" +
                                        "[?t :twitter/followersCount ?c]" +
                                        "[(> ?c 1000000)]]";
    public static final String query6 = "[:find ?name :in $ ?a1 :where " +
                                        "[?p1 :name ?a1]" + 
